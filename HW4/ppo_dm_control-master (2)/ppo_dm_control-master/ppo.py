...
class PPO():
    ...
    def learn(self):
        ...
        # 假设已经定义了 rollout 和 update 函数
        # 假设 ac_model 是您的模型
        
        for epoch in range(self.epochs):
            total_reward = self.rollout()  # 获取该epoch的总奖励
            self.update()  # 更新模型参数
        
            mean_rewards.append(total_reward)  # 记录奖励
        
            if total_reward > best_reward:  # 检查是否为最高奖励
                best_reward = total_reward  # 更新最高奖励
                best_model_path = os.path.join(self.training_path, "best_" + self.model_filename)
                torch.save(self.ac_model.state_dict(), best_model_path)  # 保存最佳模型
...
# Assume PPO and related classes (MLPActorCritic, PPOBuffer) are defined as above
