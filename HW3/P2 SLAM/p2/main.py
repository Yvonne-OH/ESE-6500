...
def run_slam(src_dir, log_dir, idx, split):
    ...
    # - log_dir: Directory to save the output map visualization.
    # - idx: Index of the dataset to process.
    # - split: Dataset split (e.g., 'train', 'test') to use.
    
    #### TODO: Checked
    
    # Initialize SLAM with specific noise parameters and resolution
    slam = slam_t(resolution=0.05, Q=np.diag([2e-4, 2e-4, 1e-4]))
    slam.read_data(src_dir, idx, split)
    total_steps = len(slam.lidar)
    
    # Initialize occupancy grid with one particle at the first lidar position
    init_pose = np.array(slam.lidar[0]['xyth'])
    init_pose[2] = slam.lidar[0]['rpy'][2]  # Use the yaw from 'rpy'
    slam.init_particles(n=1, p=init_pose.reshape((3, 1)), w=np.array([1.0]))
    slam.observation_step(t=0)
    
    # Log initialization info
    logging.info(f'> Initializing particles at first timestamp: {slam.lidar[0]["t"]}')
    
    # Re-initialize SLAM with 100 particles
    slam.init_particles(n=100)
    slam.dynamics_step(0)
    
    # Process dynamics and observations for each time step
    particle_positions = []
    ...
    # Plot and save the map and particle trajectories
    ...
    plt.show()
    fig.savefig(os.path.join(log_dir, f'Map{idx}_{split}.png'))
    plt.close(fig)
